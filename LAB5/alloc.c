...
{
	register struct hole *it; /*itator*/
	phys_clicks buffer[NR_HOLES * 2 + 1], count , i=0;
	count = mm_in.m1_i1 / (sizeof(phys_clicks) * 2 ); /* no of entries to fill */
	for(it = hole_head ; i<count && it != NIL_HOLE && it->h_base < swap_base ; it = it->h_next) {
		buffer[i++] = it->h_len;
		buffer[i++] = it->h_base;
	}
	buffer[i] = 0; /* ADDING 0 to the end of Buffer */
...
PUBLIC int do_worst_fit(void)
{
	...
	if(fit > WORST_F) /* if call argument is bigger than WORST_F ERR */
		return -1;
	FIT = fit;
	return 0;
}
 ...
 */
  ...
  register struct hole *hp, *prev_ptr, *best_one, *best_one_prev;
 ...
 case WORST_F:
  do {
  	best_one_prev = NIL_HOLE;	/* best_one_prev - best one's previos */
	best_one = hp = hole_head;	/* best_one - best one */
	while(hp != NIL_HOLE && hp->h_base < swap_base) { /*try to found bigger hole */
		if(hp->h_len > best_one->h_len) { /* we found the bigger one */
			best_one = hp;
			best_one_prev = prev_ptr;
		}
		prev_ptr = hp;
		hp = hp->h_next;
	}
	if(best_one->h_len >= clicks) {
		/* We found the biggest hole. We are using it now */
		old_base = best_one->h_base; /* remember where it started */
		best_one->h_base += clicks;  /* bite a piece off */
		best_one->h_len -= clicks;   /* ditto */

		/* Delete the hole if used up completely. */
		if(!best_one->h_len)
			del_slot(best_one_prev, best_one);      	
