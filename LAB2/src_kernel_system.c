...
/* Handle sys_fork().  m_ptr->PROC1 has forked.  The child is m_ptr->PROC2. */
  ...
  *rpc = *rpp;			/* copy 'proc' struct */
  ...
  /* ADDED BY ME */
  rpc->GROUP = rpp->GROUP; 
  rpc->BASE_PRI = rpc->REAL_PRI = rpp->BASE_PRI;
  /* CHILD TYPE = PARENT TYPE ; CHILD PRI = PARENT BASE_PRI */
...
PRIVATE int do_setpri( message *m_ptr) 
{
	...
	if (m_ptr->m1_i1 == 1)
	{
		for( tmp = BEG_USER_ADDR; tmp< END_PROC_ADDR; ++tmp)
			if (tmp->p_pid == m_ptr->m1_i3)
				break;
		tmp->REAL_PRI = tmp==NIL_PROC ? tmp-> REAL_PRI : m_ptr->m1_i2*10;
		tmp->BASE_PRI = tmp==NIL_PROC ? tmp-> BASE_PRI : m_ptr->m1_i2*10;
		
		if (tmp->GROUP == INTERACT) {
			tmp->REAL_PRI = (tmp->REAL_PRI)*3 ;
			tmp->BASE_PRI = tmp->REAL_PRI;
		}	
		if (tmp->GROUP == SAFETY){
			tmp->REAL_PRI = (tmp->REAL_PRI)*6 ;
			tmp->BASE_PRI = tmp->REAL_PRI;
		}
	
		if (tmp->REAL_PRI > MAX_PRI) tmp->REAL_PRI = MAX_PRI ;
	}
	...
	if (m_ptr->m1_i1 == 4)
	{
		for( tmp = BEG_USER_ADDR; tmp< END_PROC_ADDR; ++tmp)
			if (tmp->p_pid == m_ptr->m1_i3)
				break;
		tmp->GROUP = tmp==NIL_PROC ? tmp->GROUP : m_ptr->m1_i2 ;
	}
