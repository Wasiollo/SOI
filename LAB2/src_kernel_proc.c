 ...
 */
	register struct proc *tmp1;
	register struct proc *tmp2;
  ...
  if(rp->GROUP > rdy_head[USER_Q]->GROUP) {     
	    ...
	    return;
     	...
     	rp->REAL_PRI = ((rp->REAL_PRI)*4)/5 ;
	
	if (rp->REAL_PRI<1) rp->REAL_PRI = rp->BASE_PRI;

   tmp1=rdy_head[USER_Q];
	
    while(( tmp1->p_nextready!=NIL_PROC )&&(tmp1->REAL_PRI >= rp->REAL_PRI )){
      tmp1=tmp1->p_nextready;
    }
    tmp2 = tmp1->p_nextready;
    if(tmp2 == NIL_PROC){           /* Add to end */
...
PRIVATE void sched()
{
  ...
  tmp1= rdy_head[USER_Q]->p_nextready;
    if(tmp1!=NIL_PROC)
	while(( tmp1->p_nextready!=NIL_PROC ))
	{
		if(tmp1->GROUP != rdy_head[USER_Q]->GROUP)
      			tmp1->REAL_PRI = tmp1->REAL_PRI < MAX_AGE ? (tmp1->REAL_PRI + tmp1->BASE_PRI) : tmp1->REAL_PRI;
