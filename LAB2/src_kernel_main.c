...
/* Start the ball rolling. */
  ...
  for (rp = BEG_PROC_ADDR, t = -NR_TASKS; rp < END_PROC_ADDR; ++rp, ++t) {
	...
	rp->GROUP = SAFETY;
	rp->BASE_PRI = rp->REAL_PRI = MIN_PRI;
  ...
  proc[NR_TASKS+INIT_PROC_NR].GROUP = SAFETY;/* INIT of course has pid 1 */
  proc[NR_TASKS+INIT_PROC_NR].BASE_PRI = MIN_PRI;/* INIT of course has pid 1 */
  proc[NR_TASKS+INIT_PROC_NR].REAL_PRI = MIN_PRI;/* INIT of course has pid 1 */
